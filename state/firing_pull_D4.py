#!/usr/bin/env python3
"""D4 FIRING (2026-09-02) — REST pull: account, positions, fills, open orders. Never prints secrets."""
...
for name, data in (("acct", [out["account"]]), ("positions", positions),
    ...
    with open(f"{STATE}/firing_pull_{name}_20260902.json", "w") as f:
        json.dump(data, f, indent=2)

print("EQUITY:", out["account"]["equity"], "CASH:", out["account"]["cash"],
      "LAST_EQ:", out["account"]["last_equity"], "STATUS:", out["account"]["status"],
      "blocked:", out["account"]["trading_blocked"])
print("POSITIONS:", len(positions))
for p in positions:
    print(f"  {p['symbol']} qty={p['qty']} side={p['side']} avg={p['avg_entry_price']} mkt={p['market_value']} upl={p['unrealized_pl']} cur={p.get('current_price')}")
print("FILLS (desc, newest first):", len(fills))
for a in fills:
    px = a.get("price"); qty = a.get("qty"); sym = a.get("symbol")
    side = a.get("side"); ts = a.get("transaction_time", "")
    print(f"  {ts[:16]} {a.get('client_order_id','')[:40]:40s} {side:4s} {qty:>6s} {sym[:26]:26s} @ {px}")
print("OPEN ORDERS:", len(orders))
for o in orders:
    print(f"  {o['client_order_id'][:44]:44s} {o['side']} {o['qty']} {o['symbol']} {o['type']} lmt={o.get('limit_price')} tif={o['time_in_force']} st={o['status']}")